...
In our last lesson, we focused on **authentication** — verifying *who* a user is.
Now that we know who someone is, we need to decide *what they’re allowed to do.*
...
In this video, we’ll look at how to implement role-based authorization in PHP 8.3, how to restrict page access, and how to keep your logic clean and secure.
...
Every page request after that can check their role to decide whether they’re allowed to proceed.
...
It’s simple, clear, and works well for most applications.
...
**[Switch to code editor: `12-2-admin-dashboard.php`]**
...
Even though access is enforced on the server, it’s a good idea to hide restricted links in your navigation.
That way, users don’t see options they can’t use.
...
So far we’ve been using **Role-Based Access Control (RBAC)** — where each role has a predefined set of permissions.
...
* **`match` expressions** simplify multi-role routing:
  ...
  match ($_SESSION['role'] ?? '') {
      ...
      'staff' => include 'staff-dashboard.php',
      default => include 'intern-dashboard.php',
  };
  ```
...
* PHP 8.3 features like `match` and strict typing help make this cleaner and safer.
