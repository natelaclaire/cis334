...
Accessors (getters) and mutators (setters) are methods that allow controlled access to the properties of a class. Instead of making properties public, we keep them private or protected and provide methods to read or modify their values. This helps enforce encapsulation, allowing us to validate or transform data before it is set or returned.

### Why Use Accessors and Mutators?

- **Encapsulation:** Keeps internal data safe from unintended changes.
- **Validation:** Allows you to check or sanitize values before setting them.
- **Consistency:** Lets you update logic in one place if the way you store or retrieve data changes.

### Example

Suppose we have a `User` class with a private `$firstName` property. We use a getter to access it and a setter to change it:
...
class User {
    ...
    public function setFirstName(string $firstName): void {
        // Optionally validate or modify the value
...
echo $user->getFirstName(); // Outputs: Alice
```

In this example, the setter ensures the first name is always capitalized and trimmed. The getter provides read-only access to the property.

You can add similar methods for other properties, and include additional logic as needed (e.g., checking for valid email addresses, enforcing minimum/maximum values, etc.).
