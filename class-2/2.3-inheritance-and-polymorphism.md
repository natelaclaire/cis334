...
class Admin extends User {
    ...
    public function showPassword() {
        // ❌ Not allowed, because $password is private
        return $this->password;
...
If everything is correct, you won't see a change, but what if you misspelled `login()` as `llogin()`? You might not notice the typo, but PHP will show:
...
By the way, as our code exists at this moment, you don't need the backslash in the attribute, but it serves an important purpose: it tells PHP that you are using the built-in attribute that is defined in the global namespace. We'll talk more about namespaces next week and that will make a lot more sense then, but for now just know that even though the backslash might look weird and PHP often will work just fine without it, it's essential in certain circumstances and removes ambiguity.

***

## 9. Using the `instanceof` Keyword
