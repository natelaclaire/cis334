...
Before you begin the Reinforcement Exercises, remember to accept and merge the Pull Request that I've submitted for your weekly assignments repository, just as we did last week - if there is one to merge. I set this week up early enough that you might have already merged this week's changes when you were doing last week's.
...
11. Browse through the HTML to see how we display the chess board and the form below it. Note that we are storing the serialized `$chessBoard` object in a hidden form field and then unserializing it from the POST data. This is not at all safe and secure - in a production situation we would want to store it in a safe place such as a session variable or database that can't be modified by the end user, which we'll explore later this semester.
