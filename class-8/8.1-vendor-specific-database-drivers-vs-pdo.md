...
```php
$conn = mysqli_connect('db', 'mariadb', 'mariadb', 'mydb');
...
// Prepared SELECT
$stmt = $mysqli->prepare("SELECT vehicle_id, vin, license_plate FROM vehicles WHERE fuel_type = ?");
$fuel_type = 'hybrid';
$stmt->bind_param('s', $fuel_type); // 's' = string
$stmt->execute();
$result = $stmt->get_result();
while ($row = $result->fetch_assoc()) {
    echo $row['vehicle_id'] . ": " . $row['vin'] . " - " . $row['license_plate'] . PHP_EOL;
}
$stmt->close();

// Prepared INSERT
$ins = $mysqli->prepare("INSERT INTO vehicle_models (make, model) VALUES (?, ?)");
$make = 'Chevrolet';
$model = 'Equinox';
$ins->bind_param('ss', $make, $model);
$ins->execute();
...
$stmt = $pdo->prepare("SELECT vehicle_id, vin, license_plate FROM vehicles WHERE fuel_type = ?");
$stmt->execute(['hybrid']);
$rows = $stmt->fetchAll();
foreach ($rows as $row) {
    echo "{$row['vehicle_id']}: {$row['vin']} - {$row['license_plate']}\n";
}

// INSERT with named params
$ins = $pdo->prepare("INSERT INTO vehicle_models (make, model) VALUES (:make, :model)");
$ins->execute([
    ':make'  => 'Volkswagen',
    ':model' => 'Jetta',
]);
...
## 10) Common gotchas & debugging tips
