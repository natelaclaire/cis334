...
mysqli_stmt_bind_param($stmt, "i", $vehicle_id); // Bind the variable to the parameter, where "i" indicates integer type
$vehicle_id = 1; // Example vehicle ID
mysqli_stmt_execute($stmt); // Execute the prepared statement, no need to pass params here - the most recent bound values are used
...
```php
$stmt = $conn->prepare("SELECT service_date,total_cost FROM maintenance_records WHERE vehicle_id = ?");
$stmt->bind_param("i", $vehicle_id); // Bind the variable to the parameter, where "i" indicates integer type
$vehicle_id = 1; // Example vehicle ID
$stmt->execute(); // Execute the prepared statement, no need to pass params here - the most recent bound values are used
